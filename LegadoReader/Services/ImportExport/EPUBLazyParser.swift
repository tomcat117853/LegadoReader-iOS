...
class EPUBParsingManager {
    ...
    func getLoadedBook(id: String) -> LazyBookProtocol? {
        lock.lock()
        defer { lock.unlock() }
...
class EPUBLazyReader: BookReaderProtocol {
    ...
    private var cachedBook: LazyBookProtocol?
    
    func read(data: Data) async throws -> BookContent {
        let bookId = UUID().uuidString
        currentBookId = bookId
        
        let book = try await EPUBParsingManager.shared.loadBook(data: data, id: bookId, format: .epub)
        cachedBook = book
    ...
    func getMetadata(data: Data) -> BookMetadata {
        ...
        Task {
            ...
            if let book = try? await EPUBParsingManager.shared.loadBook(data: data, id: bookId, format: .epub) {
